...
import { Palette, GripHorizontal, Shuffle, X, ChevronLeft, ChevronRight } from 'lucide-react';
...
export const ColorPanel: React.FC<ColorPanelProps> = ({ settings, onChange, visible, onClose, isMobile }) => {
  ...
  const handlePanelMove = (e: React.PointerEvent) => {
    if (dragOffset.current && !isMobile) {
      e.preventDefault();
      e.stopPropagation();
  ...
  const calculatedSize = Math.sqrt(availableArea / targetCount) * 0.85; 
  ...
  return (
    <div 
      className={`fixed bg-zinc-900/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl flex flex-col text-zinc-200 select-none z-50 ring-1 ring-black/50 overflow-hidden ${isMobile ? 'animate-in slide-in-from-bottom-10' : ''}`}
    ...
    >
      ...
      >
        <div className="flex items-center gap-0.5 no-drag bg-black/20 rounded p-0.5">
            ...
            <span className="text-[9px] font-bold uppercase w-16 text-center text-zinc-300 select-none tracking-wide truncate">
                {currentPalette.name}
            </span>
            <button 
                onClick={nextPalette} 
        ...
        <div className="flex items-center gap-1">
            ...
            <button 
                onClick={onClose} 
                className="text-zinc-500 hover:text-white p-0.5 no-drag" 
                onPointerDown={(e) => e.stopPropagation()}
            >
      ...
      {/* Swatches Grid */}
      <div className="p-2 space-y-2 overflow-y-auto scrollbar-thin scrollbar-thumb-zinc-700 flex-1 min-h-0">
      ...
      {!isMobile && (
          <div 
            className="absolute bottom-0 right-0 w-4 h-4 cursor-nwse-resize flex items-center justify-center text-zinc-600 hover:text-white transition-colors z-50 touch-none"
